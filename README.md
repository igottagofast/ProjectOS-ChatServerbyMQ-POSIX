...
พาร์ทที่ 1 ส่วนประกาศ header, struct, และ utility เซิร์ฟเวอร์ไม่ได้ส่งให้ client ตรง ๆ ทันที แต่จะสร้าง BroadcastTask แล้วโยนเข้า broadcast_queue เพื่อให้ worker thread ไปส่งทีหลังทำให้ main thread ไม่ค้าง
ใช้ RW-lock เพราะรายการสมาชิกห้องถูกอ่านบ่อยกว่าถูกเขียน
```cpp
...
```
พาร์ทที่ 2 Global State: เก็บสถานะเซิร์ฟเวอร์ทั้งหมด ที่ทุกเธรดต้องใช้ร่วมกัน room_members จะบอกว่าตอนนี้ใครอยู่ห้องไหน ซึ่ง broadcaster ต้องอ่านจากตรงนี้
...
---
พาร์ทที่ 3: ระบบ Heartbeat และการจัดการกับ client: ให้เซิร์ฟเวอร์รู้ว่าใครยังอยู่และจัดการเอาคนที่หายไปนานออกให้
heartbeat_cleaner() จะรันเป็น thread แยกตลอดเวลา 
ซึ่งทุก ๆ 15 วินาทีเซิร์ฟเวอร์จะเช็ก client ทุกคน ถ้าไม่ส่ง heartbeat ภายใน 30 วินาที จะถือว่า client นั้นหลุดออกจากระบบแล้ว
ระบบจะเรียก handle_quit() เพื่อเคลียร์ห้อง แล้ว broadcast ว่าคนนั้นออก จากนั้นจะลบ heartbeat เซิร์ฟเวอร์สามารถจัดการ client ที่ disconnect ไปเงียบ ๆ โดยอัตโนมัติทำให้ไม่มีใครค้างอยู่ในห้อง
...
พาร์ทที่ 4 กลุ่ม Handler สำหรับคำสั่งจาก client : จะแปลงข้อความที่ client ส่งมาให้กลายเป็นการอัปเดตสถานะบวกกับถ้าต้องกระจายก็โยนงานเข้า broadcast_queue
ได้เเก่ register, join, say, dm, who, leave, quit ซึ่งทุก handler ไม่ส่งไปตรง ๆ แต่สร้างงานแล้วโยนให้ Broadcast Worker ทำให้ main thread เบาลง
...
---
พาร์ท 6 Broadcast System: Broadcast System คือส่วนที่รับผิดชอบการกระจายข้อความที่เกิดขึ้นในระบบแชท ไปยังผู้ใช้คนอื่น ๆ ที่อยู่ในห้องเดียวกัน โดยออกแบบให้แยกออกจากส่วนหลักของเซิร์ฟเวอร์ (ส่วนที่รอรับคำสั่งจาก client) เพื่อไม่ให้การส่งข้อความไปยังหลาย ๆ คนทำให้เซิร์ฟเวอร์หลักค้าง การออกแบบนี้ใช้แนวคิด Producer–Consumer: ส่วน main/handler จะเป็น Producer สร้างงานกระจายข้อความแล้วส่งเข้าคิวกลาง ส่วนกลุ่มเธรด Broadcaster จะเป็น Consumer คอยดึงงานออกมาส่งจริง
   ...
   ใช้ condition_variable ทำให้ worker ไม่ต้องวนลูปเช็คตลอด (ไม่ busy-wait)
 ```cpp
struct BroadcastTask
{
...
TaskQueue<BroadcastTask> broadcast_queue;
```
   6.3 Worker กระจายข้อความ: worker ตัวนี้คือคนงานจริงที่ทำให้ข้อความถูกส่งไปถึง client
broadcast_queue เป็นสายพานที่ส่งงานกระจายข้อความจากส่วน Router ไปยัง worker
...
```
   ...
   ตรงนี้คือจุดที่ยืนยันว่าเซิร์ฟเวอร์ไม่ได้ส่งข้อความด้วยเธรดเดียว แต่แบ่งงานออกไปให้หลายเธรดไปทำพร้อมกัน ทำให้เวลามีห้องใหญ่ ๆ หรือมี system event มาพร้อมกันหลายอัน เซิร์ฟเวอร์ยังตอบสนองต่อคำสั่งใหม่ได้ทัน เพราะ main ไม่ได้รอส่งข้อความเอง
...
Broadcast System ในโปรแกรมเซิร์ฟเวอร์นี้เป็นกลไกที่แยกการรับคำสั่งจาก client ออกจากการส่งข้อความไปยังทุกคนในห้องอย่างชัดเจน โดยใช้คิวงานส่วนกลาง (broadcast_queue) เป็นตัวกลางระหว่างส่วน Router/Handler กับกลุ่มเธรด Broadcaster วิธีนี้ทำให้ตัวเซิร์ฟเวอร์ไม่จำเป็นต้องส่งข้อความหาทุก client ด้วยตัวเองในเธรดหลัก ซึ่งจะทำให้เกิดการบล็อก แต่จะเปลี่ยนข้อความทุกชนิด (ทั้งข้อความที่ผู้ใช้พิมพ์ และข้อความระบบ เช่น JOIN/LEAVE/QUIT) ให้กลายเป็นงาน (task) แล้วให้เธรดเบื้องหลังเป็นผู้กระจายออกไปแทน ส่งผลให้ระบบสามารถรองรับผู้ใช้หลายคนและหลายห้องได้ดีขึ้น และยังขยายจำนวน worker ได้ง่ายในอนาคต

---

พาร์ทที่ 7: main() การบูตเซิร์ฟเวอร์และลูปรับข้อความ: Main System หรือที่เรียกว่า Router System เป็นศูนย์กลางการควบคุมการทำงานของเซิร์ฟเวอร์ทั้งหมด มีหน้าที่รับข้อความจาก client ทุกคน ประมวลผลคำสั่งและส่งต่อไปยังส่วนที่เกี่ยวข้อง เช่น Handler หรือ Broadcast System โดยทำงานแบบต่อเนื่องตลอดอายุการรันของโปรแกรม
...
และมี ระบบเฝ้าระวัง (Heartbeat Cleaner) แยกอีกตัวเพื่อเคลียร์ client ที่หลุด ทำให้สถานะบน server 

---
แล้ว server จัดการกับ Race Conditions อย่างไร
---
1.1 Read-Write Lock (pthread_rwlock_t)
...
คลาส TaskQueue นี้จัดการ Race Condition ภายในตัวเองโดยใช้ std::mutex เพื่อป้องกันการเข้าถึง std::queue พร้อมกัน และใช้ std::condition_variable เพื่อให้ Consumer Threads สามารถ Wait ได้อย่างมีประสิทธิภาพ (ผ่าน cv.wait()) โดยไม่สิ้นเปลือง CPU (Busy-Waiting) และจะถูก Notify เมื่อมีงานใหม่เข้ามา
...
ระบบ Client ทำหน้าที่เป็นส่วนติดต่อของผู้ใช้ (User Interface) สำหรับเชื่อมต่อและสื่อสารกับเซิร์ฟเวอร์ โดยทำหน้าที่รับและส่งข้อมูลผ่าน POSIX Message Queue ซึ่งเป็นกลไกสื่อสารระหว่างโปรเซส 
...
พาร์ทที่ 1: เธรดเบื้องหลัง: Heartbeat และ Listener
  ...
  1.1 Heartbeat Thread: เซิร์ฟเวอร์มีระบบล้าง client ที่หายไปนานแล้ว มันจะดูว่า client คนไหนไม่ส่ง PING:<name> ภายในเวลาที่กำหนดก็จะถือว่าหลุด แล้วไปลบจากห้องและ broadcast บอกคนอื่น เพราะฉะนั้น client ทุกตัวต้องมีตัวส่ง heartbeat เป็นระยะ
...
2.2 การหลีกเลี่ยงการแชร์ข้อมูล (Thread Locality)
ตัวแปรที่ใช้บัฟเฟอร์ข้อความ (msg_buffer) และตัวนับลำดับข้อความ (expected_seq) ถูกประกาศเป็น Local Variables ภายในฟังก์ชัน listen_queue

ดังนั้นมีเพียง listener_thread เท่านั้นที่สามารถเข้าถึงข้อมูลเหล่านี้ได้ เมื่อไม่มี Thread อื่นเข้ามาแย่งชิงทรัพยากร จึงไม่จำเป็นต้องใช้ Mutex ใด ๆ มาปกป้องข้อมูลชุดนี้ ซึ่งเป็นวิธีป้องกัน Race Condition ที่มีประสิทธิภาพสูงสุด

2.3 การจัดการลำดับข้อความ (Message Ordering)
จัดการ Logical Race Condition ที่เกิดจากเครือข่าย คือ Out-of-Order Delivery โดยใช้ msg_buffer และ expected_seq 

หากข้อความที่มาถึงมี seq ตรงกับ expected_seq ระบบจะแสดงผลข้อความนั้น
หาก seq มากกว่า expected_seq ระบบจะเก็บข้อความนั้นไว้ใน msg_buffer ก่อน
หลังจากแสดงผลข้อความที่ถูกต้องแล้ว ระบบจะวน Loop ตรวจสอบใน msg_buffer ว่ามีข้อความลำดับถัดไป (expected_seq ที่เพิ่มค่าแล้ว) ค้างอยู่หรือไม่ และจะแสดงผลข้อความที่ค้างไว้ตามลำดับที่ถูกต้อง
...
<img src=".\result\Screenshot 2025-11-05 113459.png" alt="Average Total Time">
 ...
 • เวลาเฉลี่ยรวมลดลงจาก 0.538 s → 0.439 s เมื่อเพิ่ม Threads จาก 1 → 2
 
 • หลังจากนั้นค่าคงที่ประมาณ 0.45 s


 • ช่วงแรกการเพิ่ม Threads ช่วยให้เวลาเฉลี่ยรวมลดลงชัดเจน เพราะมีการใช้ CPU ได้เต็มประสิทธิภาพ
 
 • แต่เมื่อเกิน 4 Threads ระบบเข้าสู่จุดสมดุล ไม่มีการเพิ่มประสิทธิภาพเพิ่มเติม
...
<img src=".\result\Screenshot 2025-11-05 113507.png" alt="Average Latency">
 ...
 • ค่าความหน่วงเฉลี่ยลดลงจาก 0.135 ms → 0.103 ms เมื่อเพิ่ม Threads จาก 1 → 2
 
 • จาก 4 Threads ขึ้นไป ค่า Latency คงที่ประมาณ 0.108 ms


 • การเพิ่ม Threads ในช่วงต้นช่วยลดเวลารอของข้อความ เนื่องจากมีการประมวลผลแบบขนานมากขึ้น
 
 • เมื่อเพิ่ม Threads มากขึ้น ประสิทธิภาพแทบไม่เปลี่ยน แสดงว่าระบบสามารถควบคุมการเข้าถึงทรัพยากรและการซิงโครไนซ์ได้ดี

---
<img src=".\result\Screenshot 2025-11-05 113515.png" alt="Average Throughput">


 • ค่าการส่งข้อความต่อวินาที (Throughput) เพิ่มขึ้นจากประมาณ 5,590 msg/sec → 6,832 msg/sec เมื่อเพิ่มจาก 1 Thread เป็น 2 Threads
 
 • หลังจากนั้นค่าจะคงที่ประมาณ 6,600 msg/sec แม้เพิ่ม Thread ถึง 64


 • แสดงให้เห็นว่าระบบสามารถขยายประสิทธิภาพได้ดีในช่วงต้น (1 → 2 Threads)
 
 • แต่หลังจากนั้นประสิทธิภาพเริ่ม “อิ่มตัว” (saturation point) เนื่องจากข้อจำกัดของ CPU, I/O หรือค่า overhead จากการจัดการ Thread
